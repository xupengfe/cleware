...
// On 0=ok, -1=error
int CUSBaccess::SetMultiConfig(int deviceNo, unsigned long int directions) {
	// 1=input, 0=output
	const int bufSize = 5;
	unsigned char buf[bufSize];
	int ok = -1;
...
int
CUSBaccess::SetCounter(int deviceNo, int counter, enum COUNTER_IDs counterID) {
	//  -1=error, COUNTER_IDs ununsed until now
	const int bufSize = 3;
	unsigned char buf[bufSize];
	int ok = -1;
	int automatic = 0;
...
int CUSBaccess::StartDevice(int deviceNo) {		// mask in case of CONTACT00-device
	int ok = 1;
	const int bufsize = 5;
	unsigned char buf[bufsize];
	static int sequenceNumber = 1;
	...
	switch ((USBtype_enum)cwGetUSBType(deviceNo)) {
		...
		case TEMPERATURE5_DEVICE: {
			...
			*timeID  = ((buf[0] & 0x3f) << 8) + buf[1];
			...
			if (valid)
				valid = (buf[4] != 0);
				// if value is > 0x100 (temp=-37,5C) there must be an error
			if (!valid) { // invalid time
				ok = 0;
				break;
				}
